...
`#ID`
: A unique label which will be passed to the role or directive as the `label` option. This **may** be defined **once**. If it is repeated, only the final label will be used and a warning will be raised. Given that `ID` **must** confirm to the `VALUE-LITERAL` pseudo-grammar in order to parse correctly, users wishing to specify more complex identifiers should pass in the `label` option explicitly as a [key-value pair](#key-value).
...
Each attribute definition **must** be separated from its neighbours by **at least** one whitespace character. Each `NAME`, `.CLASS`, and `#ID` attribute must conform to the `VALUE-LITERAL` pseudo-grammar to be syntactically valid, i.e. a series of tokens matching `[A-Za_z0-9]` separated by `-` characters.
